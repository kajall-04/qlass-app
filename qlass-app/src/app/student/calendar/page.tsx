...
const events = [
  { date: 2, title: "Physics DPP Due", color: "bg-emerald-500" },
  { date: 5, title: "Chemistry Test", color: "bg-red-500" },
  { date: 10, title: "Math Assignment", color: "bg-emerald-500" },
  { date: 15, title: "Mid-Term", color: "bg-red-500" },
  { date: 18, title: "Mid-Term End", color: "bg-red-500" },
  { date: 25, title: "Annual Day", color: "bg-violet-500" },
];
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5">
        ...
        <div className="grid grid-cols-7 gap-1">
          {cells.map((day, i) => {
            ...
            return (
              <div key={i} className={cn("min-h-[80px] p-1.5 rounded-xl", day ? "hover:bg-slate-50 dark:hover:bg-slate-800/50 cursor-pointer" : "", day === today ? "bg-emerald-50 dark:bg-emerald-900/10 ring-2 ring-emerald-500/20" : "")}>
                {day && (<>
                  <span className={cn("text-xs font-medium", day === today ? "text-emerald-600 font-bold" : "text-slate-600 dark:text-slate-400")}>{day}</span>
